...
#' @param data log-scale normalised expression vector of a gene 
#' or matrix containing all the expression values in the entire dataset
#
...
#' @param data log-scale normalised expression vector of a gene 
#' or matrix containing all the expression values in the entire dataset
#' @param alpha_fix global parameters alpha obtaining from 
#' fitting the gamma-normal mixture model to the whole dataset
#' @param beta_fix global parameters beta 
...
gammaNormFix = function(data, alpha_fix=NULL,beta_fix=NULL,thresh = 1e-7, maxiter = 10000, 
  ...
  #Return if no global parameters are set
  if(is.null(alpha_fix)|is.null(beta_fix)){
    warning("Please set the global parameters: alpha_fix, beta_fix.")
    return(NA)
  }
  
  #fitting a 2 component contrained normal-gamma mixture model
...
gammaMix = function(data, thresh = 1e-8, maxiter = 10000, 
  ...
  #fitting a Gamma-Gamma mixture model
...
##Constrained Gamma-Gamma Mixture Model
#' @param data log-scale normalised expression vector of a gene 
#' or matrix containing all the expression values in the entire dataset
#' @param alpha_fix global parameters alpha obtaining from 
#' fitting the gamma-gamma mixture model to the whole dataset
#' @param beta_fix global parameters beta 
#' 
...
gammaMixFix = function(data, alpha_fix=NULL,beta_fix=NULL,thresh = 1e-10, maxiter = 10000, 
  ...
  #Fitting a contrained Gamma-Gamma mixture model
  if (addContextData) {
    nOriginal = length(data)
    data <- c(data,contextData)
  }
...
#' @param alpha_fix global parameters alpha obtaining from 
#' fitting the gamma-gamma mixture model to the whole dataset
#' @param beta_fix global parameters beta from gamma-gamma mixture model of the whole dataset
#' @param alpha_fix_norm global parameters alpha obtaining from 
#' fitting the gamma-normal mixture model to the whole dataset
#' @param beta_fix_norm global parameters beta from gamma-normal mixture model of the whole dataset

modelSelect<-function(data,alpha_fix=NULL,beta_fix=NULL,alpha_fix_norm=NULL,beta_fix_norm=NULL,
                      area=FALSE,plot=FALSE,print=FALSE,parameter=TRUE,
  ...
  if(is.null(alpha_fix_norm)|is.null(beta_fix_norm)){
    warning("Please set the global parameters: alpha_fix_norm, beta_fix_norm.")
    return(NA)
  }
  
  #Start fitting the four mixture models
  M<-list()
  
  M[[1]]<-gammaNormMix(data,plot=plot,calculateAreaDifference = area,
                       addContextData = addContextData, contextData = contextData)
  
  M[[2]]<-gammaNormFix(data,alpha_fix = alpha_fix_norm,beta_fix=beta_fix_norm,
                       plot=plot,calculateAreaDifference = area,
                       addContextData = addContextData, contextData = contextData)
  
  M[[3]]<-gammaMix(data,plot=plot,calculateAreaDifference = area,
                          addContextData = addContextData, contextData = contextData)
  
  M[[4]]<-gammaMixFix(data,alpha_fix = alpha_fix,beta_fix=beta_fix,
                             plot=plot,calculateAreaDifference = area,
                             addContextData = addContextData, contextData = contextData)
  BIC<-NA
  colnames<-c("GN","cGN","GG","cGG")
  
  #Decide the best model and calculate the parameter
  if(sum(is.na(M[[1]]))==0&sum(is.na(M[[1]]))==0&sum(is.na(M[[1]]))==0&sum(is.na(M[[1]]))==0){
    BIC<-unlist(lapply(M,function(x)x$BIC))
    model<-which.min(BIC)
    names(BIC)<-colnames
    if(parameter){
      if(model==3|model==4){
        alpha2=M[[model]]$alpha2
        beta2=M2[[model]]$beta2
        mu=NA
        sigma=NA
        rho=M[[model]]$rho2
        mean=alpha2/beta2
      ...
      para=data.frame(colnames[model],alpha2,beta2,mu,sigma,rho,mean,sd)
      names(para)<-c("model","alpha2","beta2","mu","sigma","rho","mean","sd")
    }else{
      para=rep(NA,8)
    }
    #probExp_mat<-M[[model]]$probExpressed
  ...
  results<-list(BIC=BIC,Parameter=para)
